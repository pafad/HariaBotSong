...
bot.on("message", function(message) {
  ...
  else if(!message.author.bot && message.content.startsWith("h$annonce")) {
    message.channel.send({
      embed: {
        color: 0xFF0000,
  ...
  if (message.content.startsWith("h$botmessage") & (message.author.id == "300896265078571009")) {
    ...
    message.channel.send({
      embed: {
        color: 0xFF0000,
        description: "**:x:Erreur**\n**Il y a que le développeur qui à accès à la commande. Pour faire une annonce, demander sur les serveur officiel. Merci.**"
      }
    });
  }
